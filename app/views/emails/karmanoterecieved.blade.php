	...
	<!-- Header -->
		<table width="100%" style="text-align:left;padding:8px 10px;color: #626262;border-bottom:1px solid #ccc;margin-bottom:5px">
            <tbody>
                ...
                <th style="font-size: 14px;text-align: center;color:#626262;">You have received a KarmaNote from {{$receiverDetail->fname." ".$receiverDetail->lname}}</th>
	                </th>
	            </tr>
	        </tbody> 
        </table>
			...
			<div class="borderPic">
				<ul style="padding:0">
					<li style="list-style:none;float:left;margin-right:4px">
						<a target="_blank" href="{{$receiverDetail->linkedinurl}}"><img src="{{URL::to('/')}}/images/linkdin.png" alt="" height="20px;"></a>
					</li>
					<li style="list-style:none;float:left;margin-right:4px;position:relative">
						<a href="{{URL::to('/')}}/profile/<?php echo strtolower($receiverDetail->fname.'-'.$receiverDetail->lname).'/'.$receiverDetail->id ;?>"><img src="{{URL::to('/')}}/images/krmaicon.png" height="20px" alt=""></a>
		 ...
		 <hr style="height:1px;border:none;border-bottom:1px solid #898989;margin:10px 10%;">
			...
			<ul style="width:380px; margin:0 auto;">
				  @if (!empty($Content['skills']))
                           @foreach ($Content['skills'] as $element)
                              <span style="padding:6px 8px;color:#333;background:#ccc;border-radius:5px;font-size:13px;display:inline-block">{{$element->name}}</span>
                           @endforeach
                        @endif
			</ul>       
			<ul style="width:380px;margin:0 auto"> 
